 ...
 */
const SEOPRESS_META_KEY =
  process.env.SEOPRESS_META_KEY ?? "_seopress_pro_rich_snippets_faq";
 ...
 */
const RAW_PROGRESS_FILE =
  process.env.PROGRESS_FILE ?? join(tmpdir(), "wp-seo-mcp-progress.json");
...
if (!WP_SITE_URL || !WP_USERNAME || !WP_APP_PASSWORD) {
  process.stderr.write(
    "[wp-seo-mcp] ERROR: WP_SITE_URL, WP_USERNAME, এবং WP_APP_PASSWORD অবশ্যই set করতে হবে।\n"
  );
  process.exit(1);
}
...
/** WordPress REST API থেকে পাওয়া post object-এর shape। */
interface WPPost {
  id: number;
  link: string;
...
/** একটি FAQ item — question ও answer সহ। */
interface FaqItem {
  question: string;
  answer: string;
}
...
/**
 * Batch processing-এর একটি post-এর report entry।
 * save_progress tool দিয়ে PROGRESS_FILE এ persist করা হয়।
 */
interface ReportEntry {
  ...
  faq_count: number;
  validation: "valid" | "failed" | "not_checked" | "skipped";
  error?: string;
}

/**
 * Batch processing-এর সামগ্রিক progress।
 * completed_ids, skipped_ids, failed_ids তিনটি separate set রাখা হয়
 * যাতে interrupted run resume করা সহজ হয়।
 */
interface Progress {
 ...
 */
function validatePostId(id: unknown): number {
  const n = Number(id);
  if (!Number.isInteger(n) || n <= 0) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `id must be a positive integer, got: ${JSON.stringify(id)}`
    );
  }
  return n;
}
 ...
 */
function validateFaqItem(item: unknown, index: number): FaqItem {
  if (!item || typeof item !== "object") {
    throw new McpError(
      ErrorCode.InvalidParams,
      `faq_items[${index}] must be an object`
    );
  }
  ...
  if (typeof question !== "string" || question.trim().length === 0) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `faq_items[${index}].question must be a non-empty string`
    );
  }
  if (typeof answer !== "string" || answer.trim().length === 0) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `faq_items[${index}].answer must be a non-empty string`
    );
  }
  if (question.length > MAX_QUESTION_LENGTH) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `faq_items[${index}].question exceeds ${MAX_QUESTION_LENGTH} characters`
    );
  }
  if (answer.length > MAX_ANSWER_LENGTH) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `faq_items[${index}].answer exceeds ${MAX_ANSWER_LENGTH} characters`
    );
  }
 ...
 */
function fetchWithTimeout(ms: number): { signal: AbortSignal; clear: () => void } {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new McpError(
        ...
        `WordPress API error ${res.status} for ${url}: ${body}`
      );
    }
 ...
 */
function parseSeoPressFaq(raw: unknown): FaqItem[] {
  if (!raw) return [];
  try {
 ...
 */
function hasFaqInContent(html: string): boolean {
  if (!html) return false;
  const lower = html.toLowerCase();
  return (
    lower.includes("wp:yoast/faq-block") ||
 ...
 */
function findFaqPage(jsonLdList: unknown[]): boolean {
 ...
 */
function faqItemsToSeoPressObject(items: FaqItem[]): Record<string, FaqItem> {
 ...
 */
function buildFaqPageJsonLd(items: FaqItem[]): object {
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
 ...
 */
async function toolGetSiteInfo(): Promise<string> {
...
async function toolGetPosts(args: {
  ...
  if (args.category) qs.set("categories", String(args.category));
  ...
  const summary = posts.map((p) => {
    const faqFromMeta =
      parseSeoPressFaq(p.meta?.[SEOPRESS_META_KEY]).length > 0 ||
  ...
  return JSON.stringify(
    { page, per_page: perPage, total_posts: total, total_pages: totalPages, count: posts.length, posts: summary },
    null,
    2
  );
}
 ...
 */
async function toolGetPost(args: { id: number }): Promise<string> {
  const id = validatePostId(args.id);
  const post = (await wpFetch(
    `/posts/${id}?_fields=id,title,link,content,meta`
  )) as WPPost;
 ...
 */
async function toolHasFaq(args: { id: number }): Promise<string> {
  const id = validatePostId(args.id);
  const post = (await wpFetch(
    `/posts/${id}?_fields=id,title,link,content,meta`
  )) as WPPost;
...
async function toolSaveFaqSchema(args: {
  ...
  if (!args.allow_overwrite) {
    const post = (await wpFetch(
      `/posts/${id}?_fields=id,meta,content`
    )) as WPPost;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `SEOPress API error ${res.status} for ${url}: ${body}`
      );
    }

    const result = await res.json() as { code?: string };
    if (result.code !== "success") {
      throw new McpError(
        ErrorCode.InternalError,
        `SEOPress schemas-manual returned unexpected response: ${JSON.stringify(result)}`
      );
    }
  } catch (err) {
    if (err instanceof McpError) throw err;
...
async function toolListPostsNeedingFaq(args: {
  ...
  if (args.scan_all) {
    ...
    if (args.category) firstQs.set("categories", String(args.category));
    const { totalPages } = await wpFetchPaged(`/posts?${firstQs}`);
    toPage = totalPages;
    process.stderr.write(
      `[wp-seo-mcp] scan_all: detected ${totalPages} total pages at ${perPage}/page\n`
    );
  }

  // needing_faq: FAQ generate করতে হবে এমন posts
  // skipped: আগে থেকেই FAQ আছে এমন posts
  const needing: { id: number; title: string; url: string }[] = [];
  const skipped: { id: number; title: string; url: string }[] = [];
  ...
  for (let page = fromPage; page <= toPage; page++) {
    ...
    process.stderr.write(
      `[wp-seo-mcp] Scanned page ${page}/${toPage} — ${needing.length} need FAQ so far\n`
    );
  }
 ...
 */
async function toolValidateSchema(args: { id: number }): Promise<string> {
  const id = validatePostId(args.id);

  // Step 1: Post-এর public URL জানা
  const post = (await wpFetch(
    `/posts/${id}?_fields=id,title,link`
  )) as WPPost;
    ...
    for (const item of jsonLdList) {
      ...
      const nodes =
        obj["@type"] === "FAQPage"
          ...
          ? (obj["@graph"] as Record<string, unknown>[]).filter(
              (n) => n["@type"] === "FAQPage"
            )
          : [];
...
async function toolSaveProgress(args: {
  ...
  for (const entry of args.entries) {
    ...
    if (entry.status === "added") {
      if (!progress.completed_ids.includes(entry.id)) progress.completed_ids.push(entry.id);
      ...
      if (!progress.skipped_ids.includes(entry.id)) progress.skipped_ids.push(entry.id);
    } else if (entry.status === "failed") {
      if (!progress.failed_ids.includes(entry.id)) progress.failed_ids.push(entry.id);
 ...
 */
async function toolGetProgress(): Promise<string> {
  const progress = readProgress();
  ...
  tools: [
    {
      name: "get_site_info",
      description:
        "Return total post count, pagination info, active SEOPress meta key, and progress file path. Call this first to understand the scale of the job.",
      inputSchema: { type: "object", properties: {} },
    },
    {
      name: "get_posts",
      description:
        "Fetch a paginated list of WordPress posts. Each entry shows id, title, URL, content preview, whether it has an FAQ schema in SEOPress meta, whether the post content contains an FAQ block, and a 'needs_faq' boolean. Use this for browsing or spot-checks.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "get_post",
      description:
        "Fetch the full plain-text content, public URL, and existing FAQ schema for a single WordPress post by ID. Always call this before generating FAQ items so answers are grounded in the actual article.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "has_faq",
      description:
        "Detect whether a post already has FAQ coverage. Checks three sources: the primary SEOPress meta key, the legacy alt meta key, and Gutenberg/shortcode FAQ blocks in the post content. Returns has_faq (boolean), per-source counts, and any existing items.",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "number", description: "WordPress post ID (positive integer)" },
    ...
    },
    {
      ...
      description:
        "Write an array of FAQ items (max 5, question ≤300 chars, answer ≤2000 chars) to the SEOPress meta field of a WordPress post via the schemas-manual endpoint. Will skip if the post already has FAQ schema, unless allow_overwrite is true. Use dry_run: true to preview the payload without saving.",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "number", description: "WordPress post ID (positive integer)" },
    ...
    },
    {
      ...
      description:
        "Scan one or more pages of posts and split them into two lists: posts that need an FAQ schema vs. posts that already have one. Set scan_all: true to automatically scan every page on the site and return a complete work queue. Use this to build the queue before starting batch processing.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Fetch the live public URL of a post and check whether a valid FAQPage JSON-LD schema is present in the page source. Returns valid (boolean), faq_count_in_schema, and total JSON-LD blocks found. Use this after save_faq_schema to confirm the schema is rendering correctly.",
      inputSchema: {
        type: "object",
        properties: {
          id: { type: "number", description: "WordPress post ID (positive integer)" },
    ...
    },
    {
      name: "save_progress",
      description:
        "Persist one or more processing results to a local progress file, enabling resumable batch runs. Each entry requires id, title, url, status ('added'|'skipped'|'failed'), faq_count, and validation result. Duplicate entries are upserted by post id.",
      inputSchema: {
        type: "object",
        properties: {
          entries: {
    ...
    },
    {
      name: "get_progress",
      description:
        "Load and display the current processing state from the progress file. Returns a summary (added/skipped/failed counts), all post IDs by category, and a formatted text table of the full report. Use this to resume interrupted batch runs or generate the final report.",
      inputSchema: { type: "object", properties: {} },
    },
  ],
}));
  ...
  try {
    ...
    switch (name) {
      case "get_site_info":
        ...
        break;
      case "get_posts":
        result = await toolGetPosts(a as Parameters<typeof toolGetPosts>[0]);
        break;
      case "get_post":
        result = await toolGetPost(a as Parameters<typeof toolGetPost>[0]);
        break;
      case "has_faq":
        result = await toolHasFaq(a as Parameters<typeof toolHasFaq>[0]);
        break;
      case "save_faq_schema":
        result = await toolSaveFaqSchema(a as Parameters<typeof toolSaveFaqSchema>[0]);
        break;
      case "list_posts_needing_faq":
        result = await toolListPostsNeedingFaq(
          a as Parameters<typeof toolListPostsNeedingFaq>[0]
        );
        break;
      case "validate_schema":
        result = await toolValidateSchema(a as Parameters<typeof toolValidateSchema>[0]);
        break;
      case "save_progress":
        result = await toolSaveProgress(a as Parameters<typeof toolSaveProgress>[0]);
        break;
      case "get_progress":
...
process.stderr.write(
  `[wp-seo-mcp] Server v2.0.0 running — site: ${WP_SITE_URL} | meta key: ${SEOPRESS_META_KEY}\n`
);
